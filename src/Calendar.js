...
class Calendar extends React.Component {
  ...
  renderHeader(currentDate) {
    ...
    const prevArrow = this.props.prevArrow || <Icon name='arrow-left' type='feather' color='#424242' size={13} />
    const nextArrow = this.props.nextArrow || <Icon name='arrow-right' type='feather' color='#424242' size={13} />
    return <View style={{flexDirection: 'row', marginBottom: 15}}>
      <Text style={styles.headerTitle}>{dateFns.format(currentDate || this.state.currentDate, this.state.headerDateFormat)}</Text>
      {showArrows && <TouchableHighlight onPress={this.prevMonth} style={[styles.headerNavigation],{marginRight: 25, padding:3}}>{prevArrow}</TouchableHighlight>}
    ...
    while (day <= endDate) {
      for (let i = 0; i < 7; i++) {
        ...
        if(dateFns.isSameDay(day, selectedDate)){
          style.push(styles.daySelectedOuter)
          textViewStyle.push(styles.daySelectedView)
          textStyle.push(styles.daySelected)
          textStyle.push(styles.daySelectedText)
        }
        let itemsForTheDay = items[dateFns.format(day, 'YYYY-MM-DD')];
        days.push(
          ...
          >
            <View style={styles.dayTextWrapper}>
              <View style={textViewStyle}><Text style={textStyle}>{formattedDate}</Text></View>
              {itemsForTheDay && !dateFns.isSameDay(day, selectedDate) && <View style={[styles.dayItem, itemsForTheDay.style]}></View>}
            </View>
          </TouchableOpacity>
        );
